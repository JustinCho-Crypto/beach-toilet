...
1. **광고 시청 완료 콜백에서만 포인트 지급** (기획안 §3). 포인트 선지급 금지 — 보상형 광고 SDK의 reward callback 기준.
   - 지면 3종(발급 완료, `config.ts`): 배너 상시 하단 / 보상형 포인트 전환 / 전면형 제보 시.
   - 전면형은 `INTERSTITIAL_EVERY_N_REPORTS`(기본 3) 주기로만 노출한다. 매 제보마다 띄우면 전면형→보상형 풀스크린 2연속이 되어 앱인토스 '과도한 광고 노출' 가이드에 걸릴 수 있다. 1로 바꾸면 매번 노출.
